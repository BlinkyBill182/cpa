...
can_manage_document_collection(tid uuid) -- true for tenant_admin or manager (added in migration 0015)
```
...
`contractor` was added in migration 0012. Contractors can change `report_status` only for `client_years` where they are the assigned `contractor_id`; migration 0015 enforces that column scope with a trigger.
